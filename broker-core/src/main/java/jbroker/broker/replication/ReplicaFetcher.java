 ...
 *
 * <p>Milestone 6.2 is minimal: single partition, happy-path replication only.
 ...
 * Truncation on leadership change is 's job (OffsetsForLeaderEpoch).
 */
public final class ReplicaFetcher {
    ...
    public PollResult pollOnce(int expectedLeaderEpoch) throws IOException {
        ...
        if (resp.hasError() && resp.getError().getCode() != ErrorCodes.NONE) {
            ...
            if (code == ErrorCodes.FENCED_EPOCH) {
                log.warn(
                        ...
                        expectedLeaderEpoch);
                // will truncate and reconcile; drivers should back off.
        ...
        while (buf.remaining() >= RecordBatch.BATCH_OVERHEAD) {
            ...
            // renumbers offsets via Log.append; offsets converge only
            ...
            // will replace this with a raw-batch append that preserves
