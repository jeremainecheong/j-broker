...
- **Redis pub/sub admin event fan-out** — multi-admin-pod deployments see the same event stream ().
...
3. Coordinator assigns `member_id`, picks an assignor (round-robin in Milestone 7), returns the assignment in the heartbeat reply.
...
- **Pure step-function** — `RaftCore.step(RaftEvent) → List<RaftEffect>`. The event loop and I/O live in `RaftDriver` (raft-transport); the core is deterministic and side-effect-free, which made Milestone 3's chaos simulator trivial to build.
...
Why point-to-point instead of Raft-log-based liveness: the attempt to push `BrokerHeartbeatRecord`s through the Raft log revealed that follower-originated proposals are silently dropped (only the leader can propose). Direct RPC matches real KRaft's approach and was explicitly listed as the spec's perf-tuning fallback.
...
Log compaction keeps the latest value per key (Kafka-style). The subtle part is **sparse-offset preservation** (): a consumer holding a pre-compaction offset should still resolve to the right post-compaction record, even if every intermediate record between the consumer's offset and the surviving record was tombstoned.
...
Force-compact () lets tests and operators trigger compaction synchronously instead of waiting on the 5-minute cleaner cadence:
...
De-duplication runs on `(brokerEndpoint, brokerEventId)` so a broker event arriving via BOTH a pod's direct gRPC subscription AND a peer's Redis echo is broadcast to SSE subscribers exactly once. Hand-rolled RESP client (PUBLISH + SUBSCRIBE) matches the quota-enforcer pattern — no Jedis or Lettuce dependency.
...
- [x] **Milestone 0** — Scaffolding, ArchUnit, CI.
- [x] **Milestone 1** — Raft core: election, log replication, fsync'd persistent state, conflict-index backoff.
- [x] **Milestone 2** — Raft snapshots, membership changes, leadership transfer, pre-vote, read-index.
- [x] **Milestone 3** — Deterministic chaos simulator with crash + partition injection.
- [x] **Milestone 4** — Metadata state machine, topic CRUD, producer-ID assignment through Raft.
- [x] **Milestone 5** — Broker data plane: ProduceHandler, FetchHandler, LogManager.
- [x] **Milestone 6** — Multi-broker: replication, ISR, HWM, fenced leader epoch, acks=all, idempotent producer.
- [x] **Milestone 7** — Consumer groups: coordinator, heartbeat, offset commit/fetch, recovery from `__consumer_offsets`.
- [x] **Milestone 8** — Admin REST + UI (topics, groups, cluster, raft, events SSE).
- [x] **Milestone 9** — Observability: metrics/Prometheus/Grafana, JFR, compaction, quotas.
- [x] **Milestone 10** — Java 21 deep integration: virtual threads everywhere, zero pinning on the produce path, structured fan-out.
- [x] **Milestone 11** — Docker image hardening, RabbitMQ-style UI polish, snake_case JSON, chaos force-election.
