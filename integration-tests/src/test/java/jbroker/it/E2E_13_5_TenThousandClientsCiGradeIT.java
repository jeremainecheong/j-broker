...
/**
 * P13.5 + audit-04 — CI-grade 10k-client hot-path smoke. Mirrors
 * {@link E2E_10_1_TenThousandClientsIT}'s goal (prove the broker's
 ...
 *
 * <p>audit-04 widens the workload: instead of every client calling
 ...
 */
class E2E_13_5_TenThousandClientsCiGradeIT {
    ...
    void tenThousandConnectionsMixedWorkloadAllSucceed(@TempDir Path d1, @TempDir Path d2, @TempDir Path d3)
        ...
        try (var cluster = TestBrokerCluster.start(
            ...
            System.out.printf(
                    "E2E-13-5: %d mixed ops in %dms — produce(ok=%d fail=%d) fetch(ok=%d fail=%d) list(ok=%d fail=%d)%n",
                    TOTAL_CONNECTIONS,
                    elapsedMs,
                    produceOk.get(),
                    produceFail.get(),
