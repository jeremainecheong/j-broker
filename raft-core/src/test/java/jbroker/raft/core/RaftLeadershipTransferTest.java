 ...
 * the current leader picks a target and sends {@code TimeoutNow}. The target
 * then immediately starts an election at {@code currentTerm + 1}, skipping
 * its own randomised election timeout and the pre-vote round — the incumbent
 * leader has already vouched for it.
 */
